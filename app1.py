...
def create_tables():
    ...
    try:
        with conn:
            ...
            # NOUVELLE TABLE: Quotas de congés par employé
            cur.execute(
                """
...
def ajouter_personnel(nom, prenom, service, poste, heure_entree_prevue, heure_sortie_prevue, groupe_nuit="A"):
    conn = get_connection()
    if conn is None:
        return False
    try:
        # Convertir les objets time en chaînes de caractères
        ...
        with conn:
            ...
            cur.execute(
                """
                INSERT INTO personnels (nom, prenom, service, poste, heure_entree_prevue, heure_sortie_prevue, groupe_nuit)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (nom, prenom, service, poste, heure_entree_str, heure_sortie_str, groupe_nuit),
            )
            
            # Récupérer l'ID du nouvel employé
            personnel_id = cur.lastrowid
            
            # Initialiser le quota de congés
            cur.execute(
                """
            ...
            # Si c'est un employé de nuit, mettre à jour la table groupes_nuit_par_service
            if poste == "Nuit":
                cur.execute(
                    """
...
def modifier_personnel(personnel_id, nom, prenom, service, poste, heure_entree_prevue, heure_sortie_prevue, groupe_nuit, actif, 
                      conge_debut=None, conge_fin=None, type_conge=None, motif_conge=None):
    conn = get_connection()
    if conn is None:
        return False
    try:
        ...
        with conn:
            cur = conn.cursor()
            # Mettre à jour les informations de base du personnel
            cur.execute(
                """
                UPDATE personnels 
                SET nom = ?, prenom = ?, service = ?, poste = ?, 
                    heure_entree_prevue = ?, heure_sortie_prevue = ?, groupe_nuit = ?, actif = ?
                WHERE id = ?
                """,
                (nom, prenom, service, poste, heure_entree_str, heure_sortie_str, groupe_nuit, actif, personnel_id),
            )
            
            # Gestion des congés si les paramètres sont fournis
            if conge_debut and conge_fin and type_conge:
                # Vérifier d'abord si une période de congé existe déjà pour cet employé
                cur.execute(
                    "SELECT id FROM conges WHERE personnel_id = ? AND statut = 'Approuvé'",
                    (personnel_id,)
                )
                conge_existant = cur.fetchone()
                ...
                if conge_existant:
                    ...
                    cur.execute(
                        """
                        UPDATE conges 
                        SET date_debut = ?, date_fin = ?, type_conge = ?, motif = ?, updated_at = CURRENT_TIMESTAMP
                        WHERE id = ?
                        """,
                        (conge_debut, conge_fin, type_conge, motif_conge, conge_existant['id'])
                    )
                else:
                    ...
                    cur.execute(
                        ...
                        VALUES (?, ?, ?, ?, ?, 'Approuvé')
                        """,
                        (personnel_id, conge_debut, conge_fin, type_conge, motif_conge)
                    )
                
                # Calculer le nombre de jours de congé
                jours_conges = (conge_fin - conge_debut).days + 1
                ...
                cur.execute(
                    ...
                    WHERE personnel_id = ?
                    """,
                    (jours_conges, jours_conges, personnel_id)
                )
...
def get_personnel_par_service(groupe_nuit_actif=None):
    ...
    try:
        query = """
            SELECT id, nom, prenom, service, poste, heure_entree_prevue, heure_sortie_prevue, groupe_nuit, actif 
            FROM personnels 
            WHERE actif = 1 
        """
        ...
        if groupe_nuit_actif:
            # Pour chaque service, vérifier quel groupe est actif
            ...
            for service in services_nuit:
                ...
                if groupe_actif_service == groupe_nuit_actif:
                    conditions.append(f"(service = '{service}' AND poste = 'Nuit')")
            
            if conditions:
                query += " AND (poste = 'Jour' OR " + " OR ".join(conditions) + ")"
            else:
                query += " AND poste = 'Jour'"
        else:
            query += " AND poste = 'Jour'"
        ...
        with conn:
            ...
            if result and result['certificat_justificatif']:
                ...
                if file_type in ['jpeg', 'jpg', 'png']:
                    # Pour les images
                    st.image(io.BytesIO(file_data), caption="Certificat médical", use_column_width=True)
                    st.download_button(
...
def show_pointage_du_jour():
    ...
    if st.session_state.user_role != "admin":
        # Pour les non-admins, on utilise le groupe A par défaut
    ...
    for service, employes in personnel_filtre.items():
        ...
        for i, emp in enumerate(employes):
            ...
            # Créer un expander sans le paramètre key (qui n'est pas supporté)
...
def show_gestion_personnel():
    ...
    with tab2:
        with st.form("ajouter_personnel"):
            ...
            with col2:
                ...
                # Ajout du groupe de nuit seulement si poste = Nuit
                if poste == "Nuit":
            ...
            if st.form_submit_button("➕ Ajouter"):
                if nom and prenom and service:
                    if ajouter_personnel(nom, prenom, service, poste, heure_entree, heure_sortie, groupe_nuit):
    ...
    with tab3:
        ...
        if not personnel_actif.empty:
            ...
            if employe_selection:
                ...
                # Récupérer les informations de congé de l'employé
                conges_employe = get_conges_employe(emp_data['id'])
                conge_actuel = None
                if not conges_employe.empty and any(conges_employe['statut'] == 'Approuvé'):
                    conge_actuel = conges_employe[conges_employe['statut'] == 'Approuvé'].iloc[0]
                ...
                with st.form("modifier_personnel"):
                    ...
                    with col_c1:
                        conge_debut = st.date_input("Début du congé", value=date.today() if not conge_actuel else conge_actuel['date_debut'])
                        type_conge = st.selectbox("Type de congé", ["Congé annuel", "Maladie", "Familial", "Exceptionnel"], 
                                                index=0 if not conge_actuel else ["Congé annuel", "Maladie", "Familial", "Exceptionnel"].index(conge_actuel['type_conge']))
                    with col_c2:
                        conge_fin = st.date_input("Fin du congé", value=date.today() + timedelta(days=7) if not conge_actuel else conge_actuel['date_fin'])
                        motif_conge = st.text_area("Motif du congé", value="" if not conge_actuel else conge_actuel['motif'])
                    
                    if st.form_submit_button("💾 Enregistrer les modifications"):
                        if modifier_personnel(emp_data['id'], nom, prenom, service, poste, heure_entree, heure_sortie, groupe_nuit, actif,
                                            conge_debut, conge_fin, type_conge, motif_conge):
    ...
    with tab4:
        ...
        st.warning("⚠️ La suppression désactivera l'employé mais conservera ses données historiques.")
        
        personnel_actif = get_personnel()
        if not personnel_actif.empty:
            employe_selection = st.selectbox(
                "Sélectionner un employé à supprimer",
                personnel_actif[personnel_actif['actif'] == 1].apply(lambda x: f"{x['prenom']} {x['nom']} - {x['service']}", axis=1),
            ...
            if employe_selection:
                ...
                if st.button("🗑️ Supprimer cet employé", type="secondary"):
